...
func dateFormat(f byte, d time.Time) (string, bool) {
	switch f {
	// Date
	case 'd':
	...
	// Full Date/Time
	case 'c':
		return Date("Y-m-d\\TH:i:sP", d.Unix()), true
	case 'r':
		return Date("D, d M Y H:i:s O", d.Unix()), true
	case 'U':
...
func Date(s string, args ...interface{}) string {
	...
	for i := 0; i < s_len; i++ {
		...
		if b, ok := dateFormat(s[i], dt); ok == true {
			if pos < i {
				dst = append(dst, s[pos:i]...)
			}
			dst = append(dst, b...)
